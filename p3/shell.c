...
void procesarEntrada (char* entry, List L, Lista memL, int I, elem *P);
...
void historic (char* subCommand, List L, Lista memL, int I, elem* P) {
    ...
    if (strcmp(subCommand, "") == 0) {                                                                                  //historic
        ...
        if (strncmp(subCommand, "-r", 2) ==0) {																			//historic -rN
            ...
            if(num<=I){
                ...
                procesarEntrada(Rentry, L, memL, I, P);
...
void procesarEntrada(char* entry, List L, Lista memL, int I, elem* P) {
    ...
    if(ntrozos == 1){
        ...
        else if (strcmp(commands[0], "listprocs")==0){
            printProcesses(P);
            validCommand = true;
        }
        else if (strcmp(commands[0], "proc")==0){
            printProcesses(P);
            validCommand = true;
        }
        ...
        if((strcmp(commands[0], "authors") == 0)){
            ...
            historic(commands[1], L, memL, I, P);
            validCommand = true;
        }
        else if(strcmp(commands[0], "create") == 0) {
        ...
        else if (strcmp(commands[0], "deleteprocs") == 0){
            validCommand = true;
            if (strcmp(commands[1], "-term")==0) deleteprocsTerm(P);
            else if (strcmp(commands[1], "-sig")==0) deleteprocsSig(P);
            else validCommand = false;
        }
        else validCommand = false;
    }
    ...
    if(strcmp(commands[0], "getpriority")==0){
        if (ntrozos == 1) getPriority(0);
        else getPriority(atoi(commands[1]));
        validCommand = true;
    }

    if(strcmp(commands[0], "setpriority")==0){
        if (ntrozos == 1) getPriority(0);
        else if(ntrozos == 2) setPriority(0, atoi(commands[1]));
        else if(ntrozos == 3) setPriority(atoi(commands[1]), atoi(commands[2]));
        validCommand = true;
    }

    if(strcmp(commands[0], "getuid")==0){
        if (ntrozos == 1) getUid();
        validCommand = true;
    }

    if(strcmp(commands[0], "setuid")==0){
        if (ntrozos == 2 || ntrozos == 3){
            char *args[2];
            args[0]=commands[1];
            if(ntrozos == 3) args[1]=commands[2];
            else args[1] = NULL;
            setUid(args);
        }
        validCommand = true;
    ...
    if(strcmp(commands[0], "fork")==0){
        if (ntrozos == 1) forkWait();
        validCommand = true;
    }

    if(strcmp(commands[0], "execute")==0 || strcmp(commands[0], "background")==0 || strcmp(commands[0], "foreground")==0){
        char *argv_list[20];

        if (strncmp(commands[ntrozos-1], "@", 1) == 0){ /*si tiene @pri*/
            for(int i=0; i<ntrozos-2; i++) argv_list[i] = commands[i + 1];  //se copia el contenido de commands menos "execute"
            argv_list[ntrozos-2] = NULL;                                    //y se añade NULL como ultimo elemento
            char *pri = strtok((char *) commands[ntrozos-1], "@");

            if(strcmp(commands[0], "execute")==0) execute(commands[1], (char **) argv_list, atoi(pri));
            else if(strcmp(commands[0], "background")==0) background(commands[1], (char **) argv_list, atoi(pri), P);
            else foreground(commands[1], (char **) argv_list, atoi(pri));
        }
        else{ /*no hay @pri*/
            for(int i=0; i<ntrozos-1; i++) argv_list[i] = commands[i + 1];
            argv_list[ntrozos-1] = NULL;

            if(strcmp(commands[0], "execute")==0) execute(commands[1], (char **) argv_list, -21);//range between -20 and 20
            else if(strcmp(commands[0], "background")==0) background(commands[1], (char **) argv_list, -21, P);
            else foreground(commands[1], (char **) argv_list, -21);
        }

        validCommand = true;
    }

    if(strcmp(commands[0], "run-as")==0){
        char argv_list[100]="";
        char *login[2];

        for(int i=0; i<ntrozos-1; i++) { //se copia el contenido (menos el ligin) y se mete en argv_list
            if (i == 0 && strcmp(commands[ntrozos-1], "&") == 0) strcat(argv_list, "background");
            else if(i == 0) strcat(argv_list, "foreground");
            else{
                strcat(argv_list, " ");
                strcat(argv_list, commands[i+1]);
            }
        }
        if(strncmp(argv_list, "back", 4) == 0) strtok(argv_list, "&"); //quita &

        login[0] = "-l";
        login[1] = commands[1];

        setUid(login); //se cambia el login
        procesarEntrada(argv_list, L, memL, I, P); //se hace una llamada con argv_list

        validCommand = true;
    }

    if(strcmp(commands[0], "execute-as")==0){
        char argv_list[100]="";
        char *login[2];

        for(int i=0; i<ntrozos-1; i++) { //se copia el contenido (menos el ligin) y se mete en argv_list
            if (i == 0) strcat(argv_list, "execute");
            else{
                strcat(argv_list, " ");
                strcat(argv_list, commands[i+1]);
            }
...
int main() {
    ...
    while (!terminado) {
        ...
        if ((strcmp(entry, "quit\n") == 0) || (strcmp(entry, "end\n") == 0) || (strcmp(entry, "exit\n") == 0)) {
            ...
            procesarEntrada(entry, L, memL, I, P);
        }
        I++;
    }
    RemoveHistoric(L);
